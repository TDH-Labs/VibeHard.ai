...
- **Auth:** Clerk, env-gated on `CLERK_SECRET_KEY` + `CLERK_PUBLISHABLE_KEY`. `src/auth/clerk.ts` + `web/server.ts`. Inert without keys (legacy auth). Live sign-in verified in browser. Clerk app id `app_REDACTED_PLACEHOLDER`.
