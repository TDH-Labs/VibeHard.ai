...
1. **Tigris read/write/list from a real E2B sandbox** — confirm the actual API surface,
   auth model, and round-trip latency for a whole-tree tar of realistic size (a generated
   Next.js app + `node_modules` is not small). Not yet done.
2. ~~Nested sandbox creation from inside an already-ephemeral sandbox~~ — **CONFIRMED
   2026-07-10, live** (AC2.5). Originally scoped against Fly Machines; that spike hit
   CLI-layer friction (see SPEC's "Provider note"), which is what motivated re-scoping
   `BuildWorker` to E2B. Re-run directly against E2B via its TypeScript SDK + a raw HTTP call
   from inside a real sandbox — clean pass, no CLI involved, zero orphaned sandboxes after.
3. **Confirm `DATABASE_URL` is set in production**, not assumed — **CONFIRMED 2026-07-10**
   (`fly secrets list -a vibehard-platform` shows it deployed; checked name/digest only,
   never the value). The `Dockerfile`'s embedded-pglite fallback also lives on the same
   volume mount `fly.toml` currently pins to one machine; still relevant to the *web tier's*
   own Fly hosting (unrelated to the `BuildWorker` provider swap) — any later change to
   `min_machines_running` for the web tier needs this confirmed first, which it now is,
   though that change is still explicitly out of scope for this epic itself (SPEC "Out of
   scope").
4. **Run an actual command inside a nested E2B sandbox** (not just create+delete) — the one
   piece of AC2.5 not yet exercised live. Low risk (standard SDK code path, unrelated to the
   nesting question itself) but worth closing before treating R2 as fully verified.
