...
export function buildGenerationBrief(spec: Spec): string {
  ...
  if (spec.deployTarget === "downloadable-tool") {
    out.push(
      ...
      "- Do NOT create a `Dockerfile` meant for a deployed service, and do NOT use Supabase or any",
      "  other hosted/cloud database — if the app stores data at all, use local storage: a local",
      "  SQLite file, or plain local JSON/file-based storage for simple flat records.",
