 ...
 * (install/build/boot) into ephemeral, resource-capped Fly machines (`substrate/fly-sandbox.ts`,
 * `substrate/fly-exec-sandbox.ts`) — see `resolveSandboxHost` in `gate/verify.ts`. That removes
 * the heaviest work from the shared host entirely once `FLY_API_TOKEN` is configured in
 * production. Until then — and permanently for the gate scanners, which intentionally stay
 * host-side since they only read source as data (never execute it) — this lock is what keeps
 * concurrent heavy work from starving itself on one machine.
 */
